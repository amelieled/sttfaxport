...
type logger_data = {
  logger_name : string;  (** A name to identify who logged what. *)
}
...
let new_logger : string -> 'a logger =
 fun name ->
...
module Cli = struct
  ...
  let options : t list =
    [
      ("-I", Arg.String Api.Files.add_path, " Add folder to Dedukti path");
      ( "-d",
        Arg.Set_string indir,
        " Add directory containing Dedukti files to convert" );
      ("--debug", Arg.Set_int log_level, " Enable debug mode");
      ( "--depends",
      ...
      ("-o", Arg.String (fun s -> outdir := Some s), " Set output directory");
    ]
end
