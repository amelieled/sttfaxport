module Basic = struct
  ...
  type 'c op =
    ...
    | Subst :
        (thm * (((name * ty) list * (var * term) list) * 'b)) op
        -> (thm * 'b) op
...
module Memoized = struct
  ...
  type 'a push = { push : 'b. 'b op -> ('a * 'b) op }
  type 'a obj = { instr : 'a push; id : int }
  ...
  let impl_name () = mk_name [] "==>"
  let forall_name () = mk_name [] "!"
  let debug obj = print_op !oc (Pop (Pragma (String ("debug", load obj Empty))))
  ...
  let mk_tyOp name : tyOp obj = save { push = (fun k -> TypeOp (load name k)) }
  let arrow_tyOp () = mk_tyOp (arrow_name ())
  let bool_tyOp () = mk_tyOp (bool_name ())
  ...
  let mk_list l =
    let rec mk_list : type b. 'a obj list -> b op -> ('a list * b) op =
     fun l k ->
      match l with [] -> Nil k | x :: t -> Cons (mk_list t (load x k))
    in
    save { push = (fun k -> mk_list l k) }
  ...
  let mk_arrow_type tyl tyr : ty obj = ty_of_tyOp (arrow_tyOp ()) [ tyl; tyr ]
  let mk_bool_type () : ty obj = ty_of_tyOp (bool_tyOp ()) []
  ...
  let mk_var_term v = save { push = (fun k -> VarTerm (load v k)) }
  ...
  let const_of_name name =
    let str = string_of_name name in
    if Hashtbl.mem const_defined str then snd @@ Hashtbl.find const_defined str
    else
      (* failwith (Format.sprintf "Const %s not found" str) *)
      save { push = (fun k -> Const (load name k)) }
  ...
  let thm_of_const_name name =
    let str = string_of_name name in
    if Hashtbl.mem const_defined str then fst @@ Hashtbl.find const_defined str
    else failwith (Format.sprintf "Const %s not found" str)

  let mk_refl term = save { push = (fun k -> Refl (load term k)) }
  ...
  let mk_sym thm = save { push = (fun k -> Sym (load thm k)) }
  let mk_betaConv term = save { push = (fun k -> BetaConv (load term k)) }

  let mk_trans thml thmr =
    save { push = (fun k -> Trans (load thmr (load thml k))) }

  let mk_subst (thm : thm obj) (env_ty : (name obj * ty obj) list)
      (env_var : (var obj * term obj) list) =
    let to_pair (left, right) =
      save { push = (fun k -> Pair (load right (load left k))) }
    in
    let env_ty = mk_list (List.map to_pair env_ty) in
    let env_var = mk_list (List.map to_pair env_var) in
    let subst = to_pair (env_ty, env_var) in
  ...
  let mk_assume term = save { push = (fun k -> Assume (load term k)) }
  ...
  let mk_remove x = print_op !oc (Pop (Remove (Int (x, Empty))))
  ...
  let thm_of_lemma name =
    let str = string_of_name name in
    if Hashtbl.mem lemmas_defined str then Hashtbl.find lemmas_defined str
    else failwith (Format.sprintf "Lemma %s not found" (string_of_name name))

  let true_name () = mk_name [] "T"
  let and_name () = mk_name [] "/\\\\"

  let version () =
    print_op !oc (Version (Int (6, Empty)));
    Hashtbl.add const_defined
      (string_of_name (equal_name ()))
      ( save { push = (fun k -> Obj.magic @@ String ("Error", k)) },
        save { push = (fun k -> Const (load (equal_name ()) k)) } );
    Hashtbl.add const_defined
      (string_of_name (true_name ()))
      ( save { push = (fun k -> Obj.magic @@ String ("Error", k)) },
        save { push = (fun k -> Const (load (true_name ()) k)) } );
    Hashtbl.add const_defined
      (string_of_name (and_name ()))
      ( save { push = (fun k -> Obj.magic @@ String ("Error", k)) },
        save { push = (fun k -> Const (load (and_name ()) k)) } );
    Hashtbl.add const_defined
      (string_of_name (impl_name ()))
      ( save { push = (fun k -> Obj.magic @@ String ("Error", k)) },
        save { push = (fun k -> Const (load (impl_name ()) k)) } );
    Hashtbl.add const_defined
      (string_of_name (forall_name ()))
      ( save { push = (fun k -> Obj.magic @@ String ("Error", k)) },
        save { push = (fun k -> Const (load (forall_name ()) k)) } )
end
...
module OpenSTT = struct
  ...
  let mk_axiom_and_ () =
    ...
    let rr =
      mk_abs_term vf
        (mk_app_term (mk_app_term f (mk_true_term ())) (mk_true_term ()))
    in
    let rhs =
      mk_abs_term vx
        (mk_abs_term vy
           (mk_equal_term rl rr (mk_arrow_type binop_type (mk_bool_type ()))))
    in
  ...
  let mk_axiom_and left right =
    ...
    let rr =
      mk_abs_term fv
        (mk_app_term (mk_app_term ft (mk_true_term ())) (mk_true_term ()))
    in
    let t2 =
      mk_abs_term vy
        (mk_equal_term rl rr (mk_arrow_type binop_type (mk_bool_type ())))
  ...
  let mk_axiom_impl_ () =
    ...
    let term =
      mk_equal_term implt (mk_abs_term vx (mk_abs_term vy r)) binop_type
    in
    mk_axiom (mk_hyp []) term
  ...
  let mk_axiom_impl left right =
    ...
    let y = mk_var_term vy in
    let tl =
      mk_app_term (mk_abs_term vx (mk_abs_term vy (mk_impl_term x y))) left
    in
    let tr =
      mk_app_term
        (mk_abs_term vx
           (mk_abs_term vy
              (mk_equal_term (mk_and_term x y) x (mk_bool_type ()))))
        left
    in
    let refl = mk_refl left in
    ...
    let tr =
      mk_app_term
        (mk_abs_term vy
           (mk_equal_term (mk_and_term left y) left (mk_bool_type ())))
        right
    in
    let refl = mk_refl right in
  ...
  let mk_axiom_forall_ () =
    ...
    let instr =
      mk_equal_term tforall
        (mk_abs_term vx
           (mk_equal_term x r (mk_arrow_type ty (mk_bool_type ()))))
        (mk_arrow_type (mk_arrow_type ty (mk_bool_type ())) (mk_bool_type ()))
    in
  ...
  let mk_axiom_forall p ty =
    ...
    let tl =
      mk_app_term (mk_abs_term var (mk_forall_term (mk_var_term var) ty)) p
    in
    let tr =
      mk_app_term
        (mk_abs_term var
           (mk_equal_term (mk_var_term var)
              (mk_abs_term (mk_var (mk_name [] "x") ty) (mk_true_term ()))
              (mk_arrow_type ty (mk_bool_type ()))))
        p
    in
    let refl = mk_refl p in
    beta_equal
      (mk_appThm (mk_subst (mk_axiom_forall_ ()) env_type env_var) refl)
      tl tr
  ...
  let mk_rule_elim_forall thm lambda ty t =
    ...
    let te =
      mk_app_term (mk_abs_term (mk_var (mk_name [] "v") ty) (mk_true_term ())) t
    in
  ...
  let proj thm bool left right =
    ...
    let f = mk_var_term vf in
    let tl =
      mk_app_term (mk_abs_term vf (mk_app_term (mk_app_term f left) right)) side
    in
    let tr =
      mk_app_term
        (mk_abs_term vf
    ...
    let betaConv =
      mk_betaConv (mk_app_term (mk_abs_term x (mk_abs_term y xory)) left)
    in
    let refl = mk_refl right in
    ...
    let betaConv =
      mk_betaConv
        (mk_app_term (mk_abs_term x (mk_abs_term y xory)) (mk_true_term ()))
    in
    ...
    let tr =
      mk_app_term
        (mk_abs_term y (if bool then xory else mk_true_term ()))
        (mk_true_term ())
    in
  ...
  let proj_left thm left right = proj thm false left right
  let proj_right thm left right = proj thm true left right
