...
let rec compile_proof dkenv env proof =
  ...
  | Term.App (f, a, args) ->
      ...
      List.fold_left
        (fun (j, f') a -> compile_arg dkenv env j f' a)
        (j, f') (a :: args)
  | _ -> assert false

and compile_arg dkenv env j f' a =
  ...
  | Te (Impl (p', q')) ->
      ...
      if Sttfatyping._eq env inferred expected then (j', ImplE (j', f', a'))
      else
