...
let sanitize id =
  ...
  else if String.sub id 0 1 = "_" then "j" ^ id
  else id

let print_var oc id = Format.fprintf oc "%s" (sanitize (string_of_var id))
...
let print_dep oc dep = Format.fprintf oc "include \"%s.ma\".\n" dep
...
let rec print_arity oc arity =
  if arity = 0 then Format.fprintf oc "Type[0]"
  else Format.fprintf oc "Type[0] -> %a" print_arity (arity - 1)
...
let rec print__te oc = function
  ...
  | Abs (var, _ty, _te) ->
      Format.fprintf oc "\\lambda %a : %a. %a" print_var var print__ty _ty
        print__te _te
...
let rec print_proof oc = function
  ...
  | ImplI (_, proof, var) ->
      ...
      in
      Format.fprintf oc "\\lambda %a : %a. (%a)" print_var var print__te _te
        print_proof proof
  | ForallE (_, proof, _te) ->
  ...
  | ForallPI (_, proof, var) ->
      Format.fprintf oc "\\lambda %a : Type[0]. %a" print_var var print_proof
        proof
...
let print_item oc = function
  ...
  | Definition (name, ty, te) ->
      Format.fprintf oc "definition %a : %a := %a.@." print_name name print_ty
        ty print_te te
  | Axiom (name, te) ->
      Format.fprintf oc "axiom %a : %a.@." print_name name print_te te
  | Theorem (name, te, proof) ->
      Format.fprintf oc "definition %a : %a := %a.@." print_name name print_te
        te print_proof proof
  | TypeDecl (tyop, arity) ->
...
let string_of_item (_ : Api.Env.t) = function
  | Parameter ((_, id), ty) -> Format.asprintf "axiom %s : %a" id print_ty ty
