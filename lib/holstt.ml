module BasicHol = struct
  ...
  type tyOp = string
  type const = string
  type hol_type = Tyvar of string | Tyapp of tyOp * hol_type list
  ...
  type proof =
    ...
    | New_basic_type_definition_proof_right of
        string * (string * string) * proof
  ...
  let mk_tyOp name = name
  let arrow_tyOp () = mk_tyOp (arrow_name ())
  let bool_tyOp () = mk_tyOp (bool_name ())
  let mk_varType name = Tyvar name
  let mk_list l = l

  let ty_of_tyOp (tyop : tyOp) (tys : hol_type list) : hol_type =
    Tyapp (tyop, tys)

  let mk_arrow_type tyl tyr : hol_type = ty_of_tyOp (arrow_tyOp ()) [ tyl; tyr ]
  let mk_bool_type () : hol_type = ty_of_tyOp (bool_tyOp ()) []

  let mk_equal_type ty : hol_type =
    mk_arrow_type ty (mk_arrow_type ty (mk_bool_type ()))

  let mk_impl_type () =
    mk_arrow_type (mk_bool_type ())
      (mk_arrow_type (mk_bool_type ()) (mk_bool_type ()))

  let mk_forall_type ty =
    mk_arrow_type (mk_arrow_type ty (mk_bool_type ())) (mk_bool_type ())

  (* Building hol terms *)
  let mk_var name ty : term = Var (name, ty)
  let mk_var_term v = v
  let mk_app_term f t : term = Comb (f, t)
  let mk_abs_term v t : term = Abs (v, t)
  let term_of_const cst ty : term = Const (cst, ty)
  ...
  (* Building hol proofs *)
  let mk_refl term = Refl_proof term
  let mk_appThm pil pir = Mk_comb_proof (pil, pir)
  let mk_absThm var pi = Abs_proof (var, pi)
  let mk_sym pi = Sym_proof pi
  let mk_betaConv term = Beta_conv_proof term
  let mk_trans pil pir = Trans_proof (pil, pir)
  let mk_subst tylist tlist pi = Subst_proof ((tylist, tlist), pi)
  let mk_eqMp pil pir = Eq_mp_proof (pir, pil)
  let mk_proveHyp pil pir = Prove_hyp_proof (pil, pir)
  let mk_assume term = Assume_proof term
  let mk_deductAntiSym pil pir = Deduct_antisym_rule_proof (pil, pir)
  ...
  let mk_true_term () =
    term_of_const (const_of_name (true_name ())) (mk_true_type ())

  let mk_and () = term_of_const (const_of_name (and_name ())) (mk_and_type ())
  let mk_and_term left right = mk_app_term (mk_app_term (mk_and ()) left) right

  let mk_impl () =
    term_of_const (const_of_name (impl_name ())) (mk_impl_type ())

  let mk_impl_term left right =
    mk_app_term (mk_app_term (mk_impl ()) left) right

  let mk_forall ty =
    term_of_const (const_of_name (forall_name ())) (mk_forall_type ty)

  let mk_forall_term f ty = mk_app_term (mk_forall ty) f
  ...
  let mk_forall_equal eq ty = Mk_comb_proof (Refl_proof (mk_forall ty), eq)

  (* Prove beta-delta-conversion between terms *)
  let conv_proof comment t pi = Rule (comment ^ "CONV_CONV_rule", [ t ], [ pi ])
end
...
module PrintHol = struct
  ...
  let rec print_list s d1 d2 print_object out = function
    ...
    | hd :: q ->
        Format.fprintf out "%s%a%s%s%a" d1 print_object hd d2 s
          (print_list s d1 d2 print_object)
          q
  ...
  let rec print_type_rec out = function
    ...
    | Tyapp (tyop, tylist) when tyop = "->" ->
        Format.fprintf out "%a -> %a" print_type_atomic (List.hd tylist)
          print_type_atomic
          (List.hd (List.tl tylist))
    | Tyapp (tyop, tylist) ->
        Format.fprintf out "%s %a" tyop
          (print_list " " "" "" print_type_atomic)
          tylist

  and print_type_atomic out t =
    match t with
    | Tyapp (tyop, _) when tyop = "->" ->
        Format.fprintf out "(%a)" print_type_rec t
    | _ -> print_type_rec out t

  let print_type out t = Format.fprintf out "`:%a`" print_type_rec t
  ...
  let rec print_term_rec b out = function
    | Var (name, ty) when b ->
        Format.fprintf out "%a" (print_term_atomic b) (Var (name, ty))
    ...
    | Comb (f, t) -> (
        ...
        | Comb (Const (s, _), t1), _ when s = "==>" || s = "/\\" || s = "=" ->
            Format.fprintf out "%a %s %a" (print_term_atomic b) t1 s
              (print_term_atomic b) t
        | _ ->
            Format.fprintf out "%a %a" (print_term_atomic b) f
              (print_term_atomic b) t)
    | Abs (v, t) ->
        Format.fprintf out "\\ %a. %a" print_var v (print_term_atomic b) t

  and print_term_atomic b out t =
    match t with
    | Comb _ | Abs _ -> Format.fprintf out "(%a)" (print_term_rec b) t
    | Var _ when b -> Format.fprintf out "(%a)" print_var t
    | _ -> print_term_rec b out t

  let print_term b out t = Format.fprintf out "`%a`" (print_term_rec b) t
  ...
  let rec print_proof_rec out =
    ...
    | Thm th -> Format.fprintf out "@[%s@]@," th
    | Rule (rule, tlist, pilist) ->
        Format.fprintf out "@[<hov>%s %a @,@[<hov>%a@]@,@]@," rule
          (print_list " " "" "" print_term_proof)
          tlist
          (print_list " " "(" ")" print_proof_rec)
          pilist
    | Refl_proof t -> Format.fprintf out "@[<hov>REFL %a@]@," print_term_proof t
    | Sym_proof pi ->
        Format.fprintf out "@[<hov>SYM (%a)@]@," print_proof_rec pi
    | Trans_proof (pi1, pi2) ->
        Format.fprintf out "@;@[<hov>TRANS @[<hov>(%a)@]@, @[<hov>(%a)@]@,@]@,"
          print_proof_rec pi1 print_proof_rec pi2
    | Mk_comb_proof (pi1, pi2) ->
        Format.fprintf out "@\n@[<hov>MK_COMB (@[%a@]@,,@[%a@]@,)@]@,"
          print_proof_rec pi1 print_proof_rec pi2
    | Abs_proof (t, pi) ->
        Format.fprintf out "@[<hov>ABS %a @[(%a)@]@,@]@," print_term_proof t
          print_proof_rec pi
    | Beta_conv_proof t ->
    ...
    | Eq_mp_proof (pi1, pi2) ->
        Format.fprintf out "@[<hov>EQ_MP @[<hov>(%a)@]@, @[<hov>(%a)@]@,@]@,"
          print_proof_rec pi1 print_proof_rec pi2
    | Deduct_antisym_rule_proof (pi1, pi2) ->
        Format.fprintf out
          "@[<hov>DEDUCT_ANTISYM_RULE @[<hov>(%a)@]@, @[<hov>(%a)@]@,@]@,"
          print_proof_rec pi1 print_proof_rec pi2
    | Prove_hyp_proof (pi1, pi2) ->
        Format.fprintf out
          "@[<hov>PROVE_HYP @[<hov>(%a)@]@, @[<hov>(%a)@]@,@]@," print_proof_rec
          pi1 print_proof_rec pi2
    | Subst_proof (lists, pi) ->
        let tylist, tlist = lists in
        let print_couple_rev print_object out pair =
          let x, y = pair in
          Format.fprintf out "(%a,%a)" print_object y print_object x
        in
        Format.fprintf out "@[<hov>PINST [%a] [%a] @[<hov>(%a)@]@,@]@,"
          (print_list ";" "" "" (print_couple_rev print_type))
          tylist
          (print_list ";" "" "" (print_couple_rev print_term_proof))
          tlist print_proof_rec pi
    ...
    | New_basic_definition_proof (thm_name, name, t) ->
        Format.fprintf out "@[<hov>new_basic_definition %a@]@,;;\n\n"
          print_term_proof t;
        Format.fprintf out "let _ = (new_defs := (\"%s\",%s)::!new_defs)" name
          thm_name
    | New_basic_type_definition_proof_left (s, (s1, s2), pi) ->
        Format.fprintf out
          "@[<hov>fst (new_basic_type_definition (%s) (%s,%s) (%a))@]@," s s1 s2
          print_proof_rec pi
    | New_basic_type_definition_proof_right (s, (s1, s2), pi) ->
        Format.fprintf out
          "@[<hov>snd (new_basic_type_definition (%s) (%s,%s) (%a))@]@," s s1 s2
          print_proof_rec pi
  ...
  let print_thm_debug out = function
    | Sequent (name, hyps, t, _) ->
        Format.fprintf out "(*%s : %a |- %a*)\n" name
          (print_list ";" "" "" (print_term false))
          hyps (print_term false) t

  (* Define whole theorem *)
  let print_thm out = function
    | Sequent (name, _, _, pi) ->
        Format.fprintf out "let %s =@\n@[<v 1>@,%a@];;\n\n" name print_proof_rec
          pi
...
module HolSTT = struct
  ...
  let rec free_vartypes vars = function
    ...
    | Tyapp (_, l) ->
        List.fold_left
          (fun setvars ty -> VarsT.union setvars (free_vartypes vars ty))
          vars l
  ...
  let mk_imps (hyps : hyp) (t : term) =
    List.fold_right
      (fun x y ->
        mk_app_term
          (mk_app_term (term_of_const (impl_name ()) (mk_impl_type ())) x)
          y)
      hyps t
  ...
  let mk_comment comment = Format.fprintf !oc "------> comment : %s\n" comment
  ...
  let mk_def thm_name name eqt (ty : hol_type) =
    let thm_def =
      Sequent (name, [], eqt, New_basic_definition_proof (thm_name, name, eqt))
    in
