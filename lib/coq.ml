...
let print_dep oc dep = Format.fprintf oc "Require Import %s.\n" dep

let print_name oc (md, id) =
  let id = sanitize id in
  let md = sanitize md in
  if !cur_md = md then Format.fprintf oc "%s" id
  else Format.fprintf oc "%s.%s" md id
...
let rec print__te oc = function
  ...
  | Abs (var, _ty, _te) ->
      Format.fprintf oc "fun (%a:%a) => %a" print_var var print__ty_wp _ty
        print__te _te
