...
let are_convertible dkenv env left right =
  let leftdk = Decompile.decompile__term env.dk left in
  let rightdk = Decompile.decompile__term env.dk right in
  Api.Env.are_convertible dkenv ~ctx:env.dk leftdk rightdk
...
let _beta_reduce dkenv env _te =
  let _tedk = Decompile.decompile__term env.dk _te in
  let _tedk' =
...
module Tracer = struct
  ...
  let rec _get_redex dkenv (envl, envr) ctx (left, right) =
    ...
    | App (_tel, _ter), App (_tel', _ter') -> (
        ...
        else if is_redexable dkenv _tel' then
          get_app_redex dkenv false ctx right
        else
  ...
  let newterm dkenv env _ redex =
    ...
    | Delta (cst, _tys) ->
        ...
        let _tedk' =
          Api.Env.unsafe_reduction dkenv ~red:(ComputeStrategy.delta name) _tedk
        in
        let _tedk' =
