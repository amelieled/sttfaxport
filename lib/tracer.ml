  ...
  let newterm dkenv env _ redex =
    ...
    | Beta _te ->
        ...
        Compile_type.compile__term dkenv env
          (Api.Env.unsafe_reduction dkenv ~red:Sttfatyping.ComputeStrategy.beta_one _tedk)
    | Delta (cst, _tys) ->
        let name = name_of cst in
        ...
        let _tedk' =
          Api.Env.unsafe_reduction dkenv ~red:(Sttfatyping.ComputeStrategy.delta name) _tedk
        in
        let _tedk' =
